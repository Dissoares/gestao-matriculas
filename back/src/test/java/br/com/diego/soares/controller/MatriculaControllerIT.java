...
class MatriculaControllerIT {
    ...
    void deveListarAulasDisponiveis() {
        given()
            .when().get("/api/aluno/aulas-disponiveis")
            .then().statusCode(200)
    ...
    void deveRealizarMatricula() {
        given()
            .when().post("/api/aluno/matriculas/1")
            .then().statusCode(201)
            .body("disciplina.nome", equalTo("Programacao"));
    }
    ...
    void deveImpedirMatriculaEmAulaNaoAutorizadaParaOCurso() {
        given()
            .when().post("/api/aluno/matriculas/5")
            .then().statusCode(403);
    }
